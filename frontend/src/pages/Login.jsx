  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-tr from-pink-200 to-blue-100 px-4 py-8">
      ...
      <div className="bg-[#F8F8F4] rounded-3xl shadow-lg w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl p-6 sm:p-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 drop-shadow">Sign In</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
          <input
            ...
            disabled={loading}
          />
          {errors.emailOrUsername && <p className="text-red-500 text-sm ml-4">{errors.emailOrUsername.message}</p>}
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-3 rounded-full font-semibold text-white shadow transition-all duration-200 mt-2 ${
              loading
        ...
        <div className="text-center mt-5 text-sm">
          Don't have an account? <Link to="/signup" className="text-blue-500 underline hover:text-blue-600 transition-colors">Sign up</Link>
        </div>
